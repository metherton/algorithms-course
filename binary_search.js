...
//            var options = {
//                host: 'martinetherton.com',
//                path: '/index.html?numbers=' + listOfNumbers.join(','),
...
//
//        while (end >= start) {
//            var mid = Math.floor(new BigNumber(end).minus(start).div(2).plus(start));
//            var pivot = haystack[mid];
//
//            if (pivot === needle)
//                return mid;
//
...
//        var result = binarySearch(keysToSearch[i], items);
//        results.push(result);
//    }
//    return results.join(" ");
//};
...
BINARY_SEARCH.calculate = function(listOfNumbers, keysToSearch) {
  function bs(nums, low, high, search) {
    ...
    mid = low.plus(Math.floor((high.minus(low).div(2))));
    //     mid = low.plus(Math.floor((high.minus(low)).div(2)));
    //   mid = Math.floor(new BigNumber(high).minus(low).div(2).plus(low));
    //   console.log('mid', mid);
    var pivot = nums[mid];
    ...
    if (search === pivot) {
      return mid;
    } else if (search < pivot) {
      return bs(nums, low, mid.minus(1), search);
    } else {
      return bs(nums, mid.plus(1), high, search);
    }
  }
  var results = [];
  ...
  var items = listOfNumbers.slice(1,listOfNumbers.length);
  for (var i = 1; i < keysToSearch.length; i++) {
    var result = bs(items, new BigNumber(0), new BigNumber(items.length).minus(1), keysToSearch[i]);
    results.push(result);
  }
  return results.join(" ");
};
...
var BigNumber = (function (globalObj) {
  'use strict';
